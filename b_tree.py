...
# For Test
def b_tree_main():
    path = "data/exponential.csv"
    data = pd.read_csv(path)
    b = BTree(64) # Page Size - 64

    start_build = datetime.datetime.now()
    for index,row in data.iterrows():
        b.insert(Item(row[0], row[1]))
    print("Build Time: ",datetime.datetime.now() - start_build)

    start_predict = datetime.datetime.now()
    for index,row in data.iterrows():
        b.predict(row[0])
    print("Search Time: ",datetime.datetime.now() - start_predict)
